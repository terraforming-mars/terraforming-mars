...
generateVueClass(
  ...
  require('./build/src/components/card/CardProductionBoxComponent')
...
generateVueClass(
  ...
  require('./build/src/components/create/CorporationsFilter')
    .CorporationsFilter,
  [
    'cardsByModuleMap',
...
generateVueClass(
  ...
  require('./build/src/components/SelectHowToPayForProjectCard')
...
generateVueClass(
  ...
  require('./build/src/components/help/HelpStandardProjects')
...
function generateVueClass(name, component, dataProperties) {
  try {
    const methodNames =
      component.methods === undefined ? [] : Object.keys(component.methods);
    ...
    const propertyNames =
      component.props === undefined ? [] : Object.keys(component.props);
    ...
    if (result.errors.length > 0) {
      ...
      throw new Error(
        `errors found while parsing template for ${name}`,
        result.errors
      );
    }
    ...
    if (result.tips.length > 0) {
      ...
      throw new Error(
        `tips found while parsing template for ${name}`,
        result.tips
      );
    }
    ...
    if (i === lines.length) {
      throw new Error(
        `must provide line with 'methods: {' for inserting checker for ${name}`
      );
    }
    ...
    lines.unshift(
      'declare function _s(arg: number | string | undefined): string;'
    );
    lines.unshift('declare function _t(arg: string): unknown;');
    lines.unshift('declare function _v(arg: string): unknown;');
    lines.unshift(
      'interface VueDomEventTarget { checked: boolean, composing: boolean, value: string };'
    );
    lines.unshift(
      'interface VueDomEvent { preventDefault: () => void; target: VueDomEventTarget; };'
    );
    lines.unshift('declare function $forceUpdate(): void');
    lines.unshift(
      'declare function $set(arg1: any, key: string, value: string): void;'
    );
    // seems to be array looper iterating function needs to pass along type information
    lines.unshift(
      'declare function _l(arg1: number, arg2: (item2: number, idx: number) => any): any;'
    );
    lines.unshift(
      'declare function _l<T>(arg1: {[x: string]: T}, args: (item2: T, idx: number) => any): any;'
    );
    lines.unshift(
      'declare function _l<T>(arg1: Array<T>, arg2: (item2: T, idx: number) => any): any;'
    );
    file = lines.join('\n');
