...
export class LogHelper {
  ...
  static logTilePlacement(player: Player, space: ISpace, tileType: TileType) {
    this.logBoardTileAction(
      player,
      space,
      TileType.toString(tileType) + ' tile'
    );
  }
  ...
  static logTRIncrease(player: Player, steps: number) {
    player.game.log('${0} gained ${1} TR', (b) =>
      b.player(player).number(steps)
    );
  }

  static logVenusIncrease(player: Player, steps: number) {
    player.game.log('${0} increased Venus scale ${1} step(s)', (b) =>
      b.player(player).number(steps)
    );
  }
  ...
  static logDrawnCards(
    player: Player,
    cards: Array<ICard> | Array<CardName>,
