...
export class AresHandler {
  ...
  public static earnAdjacencyBonuses(
    ...
    if (incrementMilestone) {
      ...
      const entry: IMilestoneCount | undefined = milestoneResults.find(
        (e) => e.id === player.id
      );
      if (entry === undefined) {
        throw new Error(
          'Player ID not in the Ares milestone results map: ' + player.id
        );
      }
      entry.count++;
    }
  ...
  private static earnAdacencyBonus(
    ...
    if (
      adjacentSpace.adjacency === undefined ||
      adjacentSpace.adjacency.bonus.length === 0
    ) {
      return false;
    }
    ...
    const bonusText = bonuses
      .entries()
      .map((elem) => `${elem[1]} ${SpaceBonus.toString(elem[0])}`)
      .join(', ');
    const tileText =
      adjacentSpace.tile !== undefined
        ? TileType.toString(adjacentSpace.tile?.tileType)
        : 'no tile';
    player.game.log('${0} gains ${1} for placing next to ${2}', (b) =>
    ...
    player.game.log('${0} gains ${1} M€ for a tile placed next to ${2}', (b) =>
  ...
  public static assertCanPay(
    ...
    const cost = AresHandler.computeAdjacencyCosts(
      player.game,
      space,
    ...
    if (
      availableProductionUnits >= cost.production &&
      player.canAfford(cost.megacredits)
    ) {
      return cost;
    }
  ...
  public static payAdjacencyAndHazardCosts(
    ...
    if (cost.production > 0) {
      ...
      player.game.defer(
        new SelectProductionToLoseDeferred(player, cost.production)
      );
    }
    if (cost.megacredits > 0) {
      player.game.log('${0} placing a tile here costs ${1} M€', (b) =>
        b.player(player).number(cost.megacredits)
      );
      player.game.defer(
        new SelectHowToPayDeferred(player, cost.megacredits, {
          title: 'Select how to pay additional placement costs.',
        })
      );
    }
  }
  ...
  public static canCover(space: ISpace, newTile: ITile): boolean {
    ...
    if (
      ...
      OCEAN_UPGRADE_TILES.has(newTile.tileType)
    ) {
      return true;
    }
    return false;
  ...
  public static grantBonusForRemovingHazard(
    ...
    player.game.log("${0}'s TR increases ${1} step(s) for removing ${2}", (b) =>
      b.player(player).number(steps).string(TileType.toString(initialTileType))
    );
  }
}
