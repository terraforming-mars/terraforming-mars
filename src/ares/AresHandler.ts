...
export class AresHandler {
    ...
    public static earnAdacencyBonus(game: Game, adjacentSpace: ISpace, player: Player): boolean {
        ...
        game.log("${0} gains 1 ${1} for placing next to ${2}",
        ...
        if (adjacentSpace.player) {
            ...
            game.log("${0} gains ${1} M€ for a tile placed next to ${2}",
                b => b.player(adjacentSpace.player!).number(ownerBonus).string(tileTypeAsString(adjacentSpace.tile?.tileType)));
        }
        return true;
    }
