...
export function serializeFundedAwards(
  fundedAwards: Array<FundedAward>
): Array<SerializedFundedAward> {
...
export function deserializeFundedAwards(
  ...
  return fundedAwards.map((element: SerializedFundedAward) => {
    ...
    const award: IAward | undefined = awards.find(
      (award) => award.name === awardName
    );
    if (award === undefined) {
      throw new Error(
        `Award ${awardName} not found when rebuilding Funded Award`
      );
    }
    ...
    if (playerId === undefined) {
      throw new Error(
        `Player ID not found when rebuilding Funded Award ${awardName}`
      );
    }
    const player = players.find((player) => player.id === playerId);
    if (player === undefined) {
      throw new Error(
        `Player ${playerId} not found when rebuilding Funded Award ${awardName}`
      );
    }
