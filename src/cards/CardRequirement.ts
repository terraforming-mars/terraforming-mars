...
export class CardRequirement {
  constructor(
    ...
    private _isAny: boolean = false
  ) {}

  private amountToString(): string {
    if (
      this._type === RequirementType.OXYGEN ||
      this._type === RequirementType.VENUS
    ) {
      return `${this._amount}%`;
  ...
  public satisfies(player: Player): boolean {
    switch (this.type) {
      ...
      case RequirementType.COLONIES:
        const coloniesCount = player.game.colonies
          .map(
            (colony) =>
              colony.colonies.filter((owner) => owner === player.id).length
          )
      ...
      case RequirementType.FLOATERS:
        return this.satisfiesInequality(
          player.getResourceCount(ResourceType.FLOATER)
        );
      ...
      case RequirementType.PARTY_LEADERS:
        if (player.game.turmoil !== undefined) {
          const parties = player.game.turmoil.parties.filter(
            (party) => party.partyLeader === player.id
          ).length;
      ...
      case RequirementType.OCEANS:
        return this.checkGlobalRequirement(
          player,
      ...
      case RequirementType.RESOURCE_TYPES:
        ...
        return this.satisfiesInequality(
          standardResources + nonStandardResources
        );
      ...
      case RequirementType.COLONY_TILES:
        return this.satisfiesInequality(
          MoonExpansion.tiles(player.game, TileType.MOON_COLONY, {
            surfaceOnly: true,
            ownedBy: this._isAny ? undefined : player,
          }).length
        );

      case RequirementType.MINING_TILES:
        return this.satisfiesInequality(
          MoonExpansion.tiles(player.game, TileType.MOON_MINE, {
            surfaceOnly: true,
            ownedBy: this._isAny ? undefined : player,
          }).length
        );
      ...
      case RequirementType.ROAD_TILES:
        return this.satisfiesInequality(
          MoonExpansion.tiles(player.game, TileType.MOON_ROAD, {
            surfaceOnly: true,
            ownedBy: this._isAny ? undefined : player,
          }).length
        );
  ...
  private checkGlobalRequirement(
    player: Player,
    ...
    let playerRequirementsBonus: number =
      player.getRequirementsBonus(parameter);
...
export class TagCardRequirement extends CardRequirement {
  ...
  public satisfies(player: Player): boolean {
    ...
    if (this.tag === Tags.SCIENCE && player.hasTurmoilScienceTagBonus)
      tagCount += 1;
