...
export class BioengineeringEnclosure
  extends Card
  implements IProjectCard, IActionCard, IResourceCard
{
  constructor() {
    super({
      ...
      metadata: {
        description:
          'Requires 1 science tag to play. Add 2 animals to this card. OTHERS MAY NOT REMOVE ANIMALS FROM THIS CARD.',
        cardNumber: 'A01',
  ...
  public action(player: Player) {
    player.game.defer(
      new DeferredAction(player, () => {
        const resourceCards = player
          .getResourceCards(this.resourceType)
          .filter((card) => card.name !== CardName.BIOENGINEERING_ENCLOSURE);
        ...
        if (resourceCards.length === 1) {
          ...
          player.game.log(
            ...
            (b) => b.player(player).card(resourceCards[0])
          );
          return undefined;
        }
        ...
        return new SelectCard(
          ...
          (foundCards: Array<ICard>) => {
            ...
            player.game.log(
              '${0} moved 1 animal from Bioengineering Enclosure to ${1}.',
              (b) => b.player(player).card(foundCards[0])
            );
            return undefined;
          }
        );
