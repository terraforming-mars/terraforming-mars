...
export class EcologicalSurvey extends Card implements IProjectCard {
    ...
    super({
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          b.effect(
            'When placing a tile grants you any plants, animals or microbes, you gain one additional of each of those resources that you gain.',
            (eb) => {
  ...
  public onTilePlaced(
    ...
    // Plants
    if (
      space.bonus.includes(SpaceBonus.PLANT) ||
      this.anyAdjacentSpaceGivesBonus(cardOwner, space, SpaceBonus.PLANT) ||
      (space.tile?.tileType === TileType.OCEAN &&
        cardOwner.playedCards.some(
          (card) => card.name === CardName.ARCTIC_ALGAE
        ))
    ) {
    ...
    (
      [
        [ResourceType.MICROBE, SpaceBonus.MICROBE],
        [ResourceType.ANIMAL, SpaceBonus.ANIMAL],
      ] as [ResourceType, SpaceBonus][]
    ).forEach(([resource, bonus]) => {
      if (
        cardOwner.playedCards.some((card) => card.resourceType === resource) &&
        (space.bonus.includes(bonus) ||
          this.anyAdjacentSpaceGivesBonus(cardOwner, space, bonus))
      ) {
        cardOwner.game.defer(
          new AddResourcesToCard(cardOwner, resource, {
            logMessage: '${0} gained a bonus ${1} because of ${2}',
            logBuilder: (b) =>
              b.player(cardOwner).string(resource).cardName(this.name),
          })
        );
      }
    });
