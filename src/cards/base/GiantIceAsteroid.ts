...
export class GiantIceAsteroid extends Card implements IProjectCard {
  ...
  public canPlay(player: Player): boolean {
    const remainingOceans =
      MAX_OCEAN_TILES - player.game.board.getOceansOnBoard();
    const remainingTemperatureSteps =
      (MAX_TEMPERATURE - player.game.getTemperature()) / 2;
    const stepsRaised =
      Math.min(remainingTemperatureSteps, 2) + Math.min(remainingOceans, 2);
  ...
  public play(player: Player) {
    ...
    player.game.defer(
      new PlaceOceanTile(player, 'Select space for first ocean')
    );
    player.game.defer(
      new PlaceOceanTile(player, 'Select space for second ocean')
    );
