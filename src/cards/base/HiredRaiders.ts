    ...
    availablePlayerTargets.forEach((target) => {
      if (target.steel > 0 && !target.alloysAreProtected()) {
        ...
        availableActions.options.push(new SelectOption(optionTitle, 'Confirm', () => {
          ...
          target.deductResource(Resources.STEEL, 2, {log: true, from: player, isStealing: true});
          return undefined;
        }));
      }
      ...
      if (target.megaCredits > 0) {
        ...
        availableActions.options.push(new SelectOption(optionTitle, 'Confirm', () => {
          ...
          target.deductResource(Resources.MEGACREDITS, 3, {log: true, from: player, isStealing: true});
          return undefined;
        }));
      }
    });
