...
export class LocalHeatTrapping extends Card implements IProjectCard {
  ...
  public canPlay(player: Player): boolean {
    ...
    if (player.canUseHeatAsMegaCredits) {
      return (
        ...
        player.heat + player.megaCredits >=
          requiredHeatAmt + player.getCardCost(this)
      );
    }
  ...
  public play(player: Player) {
    const animalCards: Array<ICard> = player.getResourceCards(
      ResourceType.ANIMAL
    );
    ...
    if (animalCards.length === 0) {
      ...
      availableActions.options.push(
        new SelectOption('Gain 4 plants', 'Gain plants', gain4Plants),
        new SelectOption(
          'Add 2 animals to ' + targetCard.name,
          'Add animals',
          () => {
