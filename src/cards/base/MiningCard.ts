...
export abstract class MiningCard extends Card implements IProjectCard {
  constructor(name: CardName, cost: number, metadata: CardMetadata) {
    super({
  ...
  private isAres(): boolean {
    return (
      this.name === CardName.MINING_AREA_ARES ||
      this.name === CardName.MINING_RIGHTS_ARES
    );
  }
  private getAdjacencyBonus(
    bonusType: SpaceBonus
  ): IAdjacencyBonus | undefined {
    if (this.isAres()) {
  ...
  private getSelectTitle(): string {
    ...
    if (
      this.name === CardName.MINING_AREA ||
      this.name === CardName.MINING_AREA_ARES
    ) {
  ...
  public play(player: Player): SelectSpace {
    return new SelectSpace(
      ...
      (space: ISpace) => {
        ...
        this.bonusResource = grantTitanium
          ? Resources.TITANIUM
          : Resources.STEEL;
        this.produce(player);

        const spaceBonus = grantTitanium
          ? SpaceBonus.TITANIUM
          : SpaceBonus.STEEL;
        player.game.addTile(player, space.spaceType, space, {
          tileType: this.getTileType(spaceBonus),
        });
