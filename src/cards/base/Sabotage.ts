  ...
  public play(player: Player) {
    ...
    availablePlayerTargets.forEach((target) => {
      if (target.titanium > 0 && !target.alloysAreProtected()) {
        ...
        const optionTitle =
          'Remove ' + amountRemoved + ' titanium from ' + target.name;

        availableActions.options.push(
          new SelectOption(optionTitle, 'Confirm', () => {
            target.deductResource(Resources.TITANIUM, 3, {
              log: true,
              from: player,
            });
            return undefined;
      ...
      if (target.steel > 0 && !target.alloysAreProtected()) {
        ...
        const optionTitle =
          'Remove ' + amountRemoved + ' steel from ' + target.name;

        availableActions.options.push(
          new SelectOption(optionTitle, 'Confirm', () => {
            target.deductResource(Resources.STEEL, 4, {
              log: true,
              from: player,
            });
            return undefined;
      ...
      if (target.megaCredits > 0) {
        ...
        const optionTitle =
          'Remove ' + amountRemoved + ' M€ from ' + target.name;

        availableActions.options.push(
          new SelectOption(optionTitle, 'Confirm', () => {
            target.deductResource(Resources.MEGACREDITS, 7, {
              log: true,
              from: player,
            });
            return undefined;
    ...
    if (availableActions.options.length > 0) {
      availableActions.options.push(
        new SelectOption('Do not remove resource', 'Confirm', () => {
          return undefined;
        })
      );
