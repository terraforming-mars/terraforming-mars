...
export class GreeneryStandardProject extends StandardProjectCard {
  ...
  public canAct(player: Player): boolean {
    ...
    const oxygenNotMaxed =
      player.game.getOxygenLevel() < constants.MAX_OXYGEN_LEVEL;
    if (
      PartyHooks.shouldApplyPolicy(player.game, PartyName.REDS) &&
      oxygenNotMaxed
    )
      greeneryCost += REDS_RULING_POLICY_COST;

    return (
      player.canAfford(greeneryCost) &&
  ...
  actionEssence(player: Player): void {
    player.game.defer(
      new PlaceGreeneryTile(player, 'Select space for greenery')
    );
  }
}
