...
export class Aridor extends Card implements CorporationCard {
    ...
    super({
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          ...
          b.corpBox('effect', (ce) => {
            ce.effect(
              'When you get a new type of tag in play [event cards do not count], increase your M€ production 1 step.',
              (eb) => {
                eb.diverseTag().startEffect.production((pb) =>
                  pb.megacredits(1)
                );
              }
            );
  ...
  public initialAction(player: Player) {
    ...
    const coloniesModel: Array<ColonyModel> =
      game.getColoniesModel(availableColonies);
        ...
        if (game.colonyDealer !== undefined) {
          availableColonies.forEach((colony) => {
            if (colony.name === colonyName) {
              ...
              game.log('${0} added a new Colony tile: ${1}', (b) =>
  ...
  private checkActivation(colony: Colony, game: Game): void {
    ...
    game.getPlayers().forEach((player) => {
      if (
        ...
        player.corporationCard.resourceType === colony.resourceType
      ) {
        colony.isActive = true;
        return;
      }
      const resourceCard = player.playedCards.find(
        (card) => card.resourceType === colony.resourceType
      );
