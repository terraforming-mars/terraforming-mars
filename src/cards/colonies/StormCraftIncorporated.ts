...
export class StormCraftIncorporated
  extends Card
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          ...
          b.corpBox('action', (ce) => {
            ...
            ce.vSpace();
            ce.effect(
              'Floaters on this card may be used as 2 heat each.',
              (eb) => {
  ...
  public spendHeat(
    ...
    return new AndOptions(
      () => {
        ...
        if (
          heatAmount > 0 &&
          heatAmount - 1 + floaterAmount * 2 >= targetAmount
        ) {
          throw new Error(`You cannot overspend heat`);
        }
        if (
          floaterAmount > 0 &&
          heatAmount + (floaterAmount - 1) * 2 >= targetAmount
        ) {
          throw new Error(`You cannot overspend floaters`);
        }
        player.removeResourceFrom(
          player.corporationCard as ICard,
          floaterAmount
        );
      ...
      new SelectAmount(
        ...
        0,
        Math.min(player.heat, targetAmount)
      ),
      new SelectAmount(
