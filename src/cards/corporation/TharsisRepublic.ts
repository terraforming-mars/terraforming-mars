...
export class TharsisRepublic extends Card implements CorporationCard {
    ...
    super({
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          ...
          b.corpBox('effect', (ce) => {
            ce.effect(
              'When any city tile is placed ON MARS, increase your M€ production 1 step. When you place a city tile, gain 3 M€.',
              (eb) => {
  ...
  public initialAction(player: Player) {
    return new SelectSpace(
      'Select space on mars for city tile',
    ...
    if (Board.isCitySpace(space)) {
      if (cardOwner.id === activePlayer.id) {
        cardOwner.game.defer(
          new GainResources(cardOwner, Resources.MEGACREDITS, {count: 3})
        );
      }
