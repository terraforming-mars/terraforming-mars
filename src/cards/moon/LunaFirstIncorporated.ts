  ...
  public readonly metadata: CardMetadata = {
    ...
    renderData: CardRenderer.builder((b) => {
      ...
      b.effect('When you raise any Moon Rate, increase your MC production 1 step per step.', (eb) => {
        eb.moonColonyRate({size: 'small'}).slash()
          .moonMiningRate({size: 'small'}).slash()
          .moonLogisticsRate({size: 'small'})
          .startEffect.production((pb) => pb.megacredits(1));
      }).br,
      b.effect('When any player raises any Moon Rate, gain 1MC per step.', (eb) => {
        eb.moonColonyRate({size: 'small'}).any.slash()
          .moonMiningRate({size: 'small'}).any.slash()
          .moonLogisticsRate({size: 'small'}).any.
