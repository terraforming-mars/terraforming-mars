...
export class LunarMineUrbanization extends Card implements IProjectCard {
  constructor() {
    super({
      ...
      metadata: {
        description:
          'Requires you have 1 mine tile. Increase your M€ production 1 step. Replace one of your mine tiles ' +
  ...
  public play(player: Player) {
    ...
    const tiles = MoonExpansion.tiles(player.game, TileType.MOON_MINE, {
      ownedBy: player,
    });
    return new SelectSpace(
      ...
      (space) => {
        if (space.tile === undefined) {
          throw new Error(
            `Space ${space.id} should have a tile, how doesn't it?`
          );
        }
