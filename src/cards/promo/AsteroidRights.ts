  ...
  implements IActionCard, IProjectCard, IResourceCard
{
  ...
  public action(player: Player) {
    ...
    const spendAsteroidOption = new SelectOption(
      'Remove 1 asteroid on this card to increase M€ production 1 step OR gain 2 titanium',
      'Remove asteroid',
      () => {
        this.resourceCount--;
    ...
    const addAsteroidToSelf = new SelectOption(
      'Add 1 asteroid to this card',
      'Add asteroid',
      () => {
        player.game.defer(
          new SelectHowToPayDeferred(player, 1, {
            title: 'Select how to pay for asteroid',
          })
        );
    ...
    asteroidCards.length === 1
      ? opts.push(addAsteroidToSelf)
