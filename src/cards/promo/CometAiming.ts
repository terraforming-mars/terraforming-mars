  ...
  implements IActionCard, IProjectCard, IResourceCard
{
    ...
    super({
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          b.action(
            'Spend 1 titanium to add 1 asteroid resource to ANY CARD.',
            (eb) => {
  ...
  public canAct(player: Player): boolean {
    ...
    if (PartyHooks.shouldApplyPolicy(player.game, PartyName.REDS)) {
      return (
        hasTitanium ||
        (player.canAfford(REDS_RULING_POLICY_COST) && canPlaceOcean)
      );
    }
  ...
  public action(player: Player) {
    ...
    if (availableActions.length === 1) {
      ...
      if (action instanceof SelectOption)
        return (availableActions[0] as SelectOption).cb();
