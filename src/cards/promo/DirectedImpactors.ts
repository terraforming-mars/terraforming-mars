...
export class DirectedImpactors
  extends Card
  implements IActionCard, IProjectCard, IResourceCard
{
    ...
    super({
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          b.action(
            'Spend 6 M€ to add 1 asteroid to ANY CARD (titanium may be used to pay for this).',
            (eb) => {
              eb.megacredits(6)
                .titanium(1)
                .brackets.startAction.asteroids(1)
                .asterix();
            }
          ).br;
          b.or().br;
          b.action(
            'Remove 1 asteroid here to raise temperature 1 step.',
            (eb) => {
  ...
  public action(player: Player) {
    ...
    const addResource = new SelectOption(
      'Pay 6 to add 1 asteroid to a card',
      'Pay',
      () => this.addResource(player, asteroidCards)
    );
  ...
  private addResource(player: Player, asteroidCards: ICard[]) {
    player.game.defer(
      new SelectHowToPayDeferred(player, 6, {
        canUseTitanium: true,
        title: 'Select how to pay for Directed Impactors action',
      })
    );
