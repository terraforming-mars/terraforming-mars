...
export class PharmacyUnion extends Card implements CorporationCard {
    ...
    super({
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          ...
          b.text(
            '(You start with 54 M€ . Draw a Science card.)',
            Size.TINY,
            false,
            false
          );
          b.corpBox('effect', (ce) => {
            ...
            ce.vSpace();
            ce.effect(
              'When ANY microbe tag is played, add a disease here and lose 4 M€ or as much as possible. When you play a science tag, remove a disease here and gain 1 TR OR if there are no diseases here, you MAY put this card face down in your EVENTS PILE to gain 3 TR.',
              (eb) => {
  ...
  private _onCardPlayed(
    player: Player,
    card: IProjectCard | CorporationCard
  ): void {
    ...
    if (
      ...
      if (
        !redsAreRuling ||
        (redsAreRuling &&
          player.canAfford(REDS_RULING_POLICY_COST * 3) === true)
      ) {
        game.defer(
          new DeferredAction(player, () => {
            const orOptions = new OrOptions(
              new SelectOption(
                'Turn it face down to gain 3 TR and lose up to 4 M€',
                'Confirm',
                () => {
              ...
              new SelectOption(
                'Add a disease to it and lose up to 4 M€, then remove a disease to gain 1 TR',
                'Confirm',
                () => {
                  const megaCreditsLost = Math.min(player.megaCredits, 4);
            ...
            orOptions.title =
              'Choose the order of tag resolution for Pharmacy Union';
            return orOptions;
          }),
          -1
        ); // Make it a priority
    ...
    if (isPharmacyUnion && hasScienceTag) {
      const scienceTags = card.tags.filter(
        ...
        game.defer(
          new DeferredAction(player, () => {
            ...
            if (this.resourceCount > 0) {
              if (
                redsAreRuling &&
                player.canAfford(REDS_RULING_POLICY_COST) === false
              ) {
                ...
                this.resourceCount--;
                player.increaseTerraformRating();
                game.log('${0} removed a disease from ${1} to gain 1 TR', (b) =>
            ...
            if (
              redsAreRuling &&
              player.canAfford(REDS_RULING_POLICY_COST * 3) === false
            ) {
              // TODO (Lynesth): Remove this when #1670 is fixed
              game.log(
            ...
            return new OrOptions(
              new SelectOption(
                'Turn this card face down and gain 3 TR',
                'Gain TR',
                () => {
                  this.isDisabled = true;
                  player.increaseTerraformRatingSteps(3);
                  game.log('${0} turned ${1} face down to gain 3 TR', (b) =>
                    b.player(player).card(this)
                  );
                  return undefined;
                }
    ...
    if (hasMicrobesTag) {
      game.defer(
        new DeferredAction(player, () => {
          const microbeTagCount = card.tags.filter(
            (cardTag) => cardTag === Tags.MICROBE
          ).length;
          const player = game
            .getPlayers()
            .find((p) => p.isCorporation(this.name))!;
          const megaCreditsLost = Math.min(
            player.megaCredits,
            microbeTagCount * 4
          );
          player.addResourceTo(this, microbeTagCount);
          player.megaCredits -= megaCreditsLost;
          game.log('${0} added a disease to ${1} and lost ${2} M€', (b) =>
            b.player(player).card(this).number(megaCreditsLost)
          );
          return undefined;
        }),
        -1
