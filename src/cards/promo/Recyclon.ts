...
export class Recyclon extends Card implements CorporationCard, IResourceCard {
    ...
    super({
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          ...
          b.corpBox('effect', (ce) => {
            ce.effect(
              'When you play a building tag, including this, gain 1 microbe to this card, or remove 2 microbes here and raise your plant production 1 step.',
              (eb) => {
                eb.building().played.colon().microbes(1).or();
                eb.microbes(2).digit.startEffect.production((pb) =>
                  pb.plants(1)
                );
              }
            );
  ...
  public onCardPlayed(player: Player, card: IProjectCard) {
    if (
      card.tags.includes(Tags.BUILDING) === false ||
