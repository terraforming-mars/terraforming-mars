...
export class SelfReplicatingRobots extends Card implements IProjectCard {
  ...
  public canAct(player: Player): boolean {
    return (
      this.targetCards.length > 0 ||
      player.cardsInHand.some((card) =>
        card.tags.some((tag) => tag === Tags.SPACE || tag === Tags.BUILDING)
      )
    );
  }
  ...
  public action(player: Player) {
    ...
    const selectableCards = player.cardsInHand.filter((card) =>
      card.tags.some((tag) => tag === Tags.SPACE || tag === Tags.BUILDING)
    );
    ...
    if (selectableCards.length > 0) {
      orOptions.options.push(
        new SelectCard(
          ...
          selectableCards,
          (foundCards: Array<IProjectCard>) => {
            const projectCardIndex = player.cardsInHand.findIndex(
              (card) => card.name === foundCards[0].name
            );
            ...
            player.game.log('${0} linked ${1} with ${2}', (b) =>
              b.player(player).card(foundCards[0]).card(this)
            );
            return undefined;
          }
        )
