...
export class BannedDelegate extends Card implements IProjectCard {
    ...
    player.game.turmoil!.parties.forEach((party) => {
      if (party.delegates.length > 1) {
        ...
        const playersId = Array.from(
          new Set<PlayerId | NeutralPlayer>(delegates)
        );
        ...
        if (players.length > 0) {
          const selectDelegate = new SelectDelegate(
            players,
            'Select player delegate to remove from ' + party.name + ' party',
            (selectedPlayer: Player | NeutralPlayer) => {
              let playerToRemove = '';
              ...
              player.game.turmoil!.removeDelegateFromParty(
                playerToRemove,
                party.name,
                player.game
              );
  ...
  private log(
    ...
    if (selectedPlayer === 'NEUTRAL') {
      player.game.log('${0} removed neutral delegate from ${1}', (b) =>
        b.player(player).party(party)
      );
    } else {
      player.game.log("${0} removed ${1}'s delegate from ${2}", (b) =>
        b.player(player).player(selectedPlayer).party(party)
      );
    }
  }
}
