...
export class GMOContract extends Card implements IProjectCard {
    ...
    super({
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          b.effect(
            'Each time you play a plant, animal or microbe tag, including this, gain 2 M€.',
            (be) => {
              be.animals(1).played.slash().plants(1).played.slash().microbes(1)
                .played;
  ...
  public onCardPlayed(player: Player, card: IProjectCard): void {
    const amount = card.tags.filter(
      (tag) => tag === Tags.ANIMAL || tag === Tags.PLANT || tag === Tags.MICROBE
    ).length;
    if (amount > 0) {
      player.game.defer(
