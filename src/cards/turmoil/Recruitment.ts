...
export class Recruitment extends Card implements IProjectCard {
  ...
  public canPlay(player: Player): boolean {
    if (
      player.game.turmoil === undefined ||
      player.game.turmoil.hasAvailableDelegates(player.id) === false
    ) {
      return false;
    }
    ...
    return player.game.turmoil.parties.some((party) => {
      ...
      return (
        neutralDelegates > 1 ||
        (neutralDelegates === 1 && party.partyLeader !== 'NEUTRAL')
      );
    });
  }
  ...
  public play(player: Player) {
    player.game.defer(
      new SendDelegateToArea(
        ...
        {replace: 'NEUTRAL', source: 'reserve'}
      )
    );
    return undefined;
  }
