...
export class CometForVenus extends Card {
  constructor() {
    super({
      ...
      metadata: {
        description:
          'Raise Venus 1 step. Remove up to 4M€ from any player WITH A VENUS TAG IN PLAY.',
        cardNumber: '218',
        renderData: CardRenderer.builder((b) => {
          b.venus(1).nbsp.nbsp.minus().megacredits(4).secondaryTag(Tags.VENUS)
            .any;
        }),
      },
    });
  ...
  public play(player: Player) {
    const venusTagPlayers = player.game
      .getPlayers()
      .filter(
        (otherPlayer) =>
          otherPlayer.id !== player.id &&
          otherPlayer.getTagCount(Tags.VENUS, false, false) > 0
      );
    ...
    if (venusTagPlayers.length > 0) {
      return new OrOptions(
        ...
        new SelectOption('Do not remove M€', 'Confirm', () => {
          player.game.increaseVenusScaleLevel(player, 1);
          return undefined;
        })
      );
