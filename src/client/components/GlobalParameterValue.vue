...
const isMax = computed((): boolean => {
  return props.value === attributes[props.param].max;
});

const title = computed((): string => {
  return attributes[props.param].title;
});

const iconClass = computed((): string => {
  return attributes[props.param].iconClass;
});

const suffix = computed((): string => {
  return props.param === GlobalParameter.OXYGEN ? '%' : '';
});
</script>
