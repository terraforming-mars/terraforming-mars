<template>
  <div id="player-home" :class="game.turmoil ? 'with-turmoil' : ''">
    ...
    <div v-if="thisPlayer.tableau.length > 0" style="display: flex">
      <div style="padding-right: 50px">
        <div class="player_home_block">
          ...
          <MoonBoard
            v-if="game.gameOptions.moonExpansion"
            :model="game.moon"
            :tileView="tileView"
          ...
          <PlanetaryTracks
            v-if="game.gameOptions.pathfindersExpansion"
            :tracks="game.pathfinders"
            :gameOptions="game.gameOptions"
          />

          <div
            v-if="playerView.players.length > 1"
            class="player_home_block--milestones-and-awards"
          >
      ...
      <div>
        ...
        >
          <div class="hiding-card-button-row">
            ...
            <div
              :class="getHideButtonClass('HAND')"
              v-on:click.prevent="toggle('HAND')"
            >
              <div class="played-cards-count">
                {{ cardsInHandCount.toString() }}
              </div>
              <div class="played-cards-selection" v-i18n>
                {{ getToggleLabel("HAND") }}
              </div>
            </div>
        ...
        <div class="player_home_block player_home_block--cards">
          <div class="hiding-card-button-row">
            ...
            <div class="played-cards-filters">
              <div
                :class="getHideButtonClass('ACTIVE')"
                v-on:click.prevent="toggle('ACTIVE')"
              >
                <div class="played-cards-count">
                  {{
              ...
              <div
                :class="getHideButtonClass('AUTOMATED')"
                v-on:click.prevent="toggle('AUTOMATED')"
              >
                <div class="played-cards-count">
                  {{
                    getCardsByType(thisPlayer.tableau, [
                ...
                <div class="played-cards-selection" v-i18n>
                  {{ getToggleLabel("AUTOMATED") }}
                </div>
              </div>
              <div
                :class="getHideButtonClass('EVENT')"
          ...
          <div
            v-for="card in getCardsByType(thisPlayer.tableau, [
          ...
          >
            <Card
              :card="card"
              :actionUsed="isCardActivated(card, thisPlayer)"
            />
          </div>
          <div
            v-for="card in getCardsByType(thisPlayer.tableau, [CardType.CEO])"
            :key="card.name"
            class="cardbox"
          >
            <Card
          ...
          <div
            ...
            v-for="card in sortActiveCards(
              getCardsByType(thisPlayer.tableau, [CardType.ACTIVE])
            )"
            :key="card.name"
            class="cardbox"
          >
        ...
        <div
          v-if="thisPlayer.selfReplicatingRobotsCards.length > 0"
          class="player_home_block"
        >
          <dynamic-title
            title="Self-Replicating Robots cards"
            :color="thisPlayer.color"
          />
          <div>
            <div
              v-for="card in thisPlayer.selfReplicatingRobotsCards"
              :key="card.name"
              class="cardbox"
            >
              <Card :card="card" />
      ...
      <div
        class="player_home_block player_home_block--setup nofloat"
        v-if="thisPlayer.tableau.length === 0"
      >
        <template v-if="isInitialDraftingPhase()">
          <div
            v-for="card in playerView.dealtCorporationCards"
            :key="card.name"
            class="cardbox"
          >
            <Card :card="card" />
          ...
          <div
            v-for="card in playerView.dealtPreludeCards"
            :key="card.name"
            class="cardbox"
          >
            <Card :card="card" />
          ...
          <div
            v-for="card in playerView.dealtCeoCards"
            :key="card.name"
            class="cardbox"
          >
            <Card :card="card" />
          ...
          <div
            v-for="card in playerView.dealtProjectCards"
            :key="card.name"
            class="cardbox"
          >
            <Card :card="card" />
        ...
        <template v-if="playerView.pickedCorporationCard.length === 1">
          ...
          <div>
            ...
            <template v-if="game.gameOptions.preludeExtension">
              <div
                v-for="card in playerView.preludeCardsInHand"
                :key="card.name"
                class="cardbox"
              >
                <Card :card="card" />
            ...
            <template v-if="game.gameOptions.ceoExtension">
              <div
                v-for="card in playerView.ceoCardsInHand"
                :key="card.name"
                class="cardbox"
              >
                <Card :card="card" />
          ...
          <div>
            <div
              v-for="card in playerView.cardsInHand"
              :key="card.name"
              class="cardbox"
            >
              <Card :card="card" />
        ...
        <dynamic-title
          v-if="playerView.pickedCorporationCard.length === 0"
          title="Select initial cards:"
          :color="thisPlayer.color"
        />
        <waiting-for
          v-if="game.phase !== 'end'"
        ...
        <div
          class="player_home_block player_home_block--turnorder nofloat"
          v-if="playerView.players.length > 1"
        >
          ...
          >
            <div
              class="player_name_cont"
              :class="getPlayerCssForTurnOrder(p, true)"
            >
              <span class="player_number">{{ idx + 1 }}.</span
              ><span
                class="player_name"
                :class="getPlayerCssForTurnOrder(p, false)"
                href="#"
                >{{ p.name }}</span
              >
            </div>
            <div
              class="player_separator"
              v-if="idx !== playerView.players.length - 1"
            >
              ⟶
            </div>
          </div>
    ...
    <div v-if="game.spectatorId">
      <a
        :href="'/spectator?id=' + game.spectatorId"
        target="_blank"
    ...
    <purge-warning
      :expectedPurgeTimeMs="playerView.game.expectedPurgeTimeMs"
    ></purge-warning>
  </div>
</template>
...
import { TileView, nextTileView } from "./board/TileView";
...
export default Vue.extend({
  ...
  watch: {
    ...
    showActiveCards: function toggle_active_cards() {
      PreferencesManager.INSTANCE.set(
        "hide_active_cards",
  ...
  methods: {
    ...
    isPlayerActing(playerView: PlayerViewModel): boolean {
      return (
        playerView.players.length > 1 && playerView.waitingFor !== undefined
      );
    },
    ...
    isInitialDraftingPhase(): boolean {
      return (
        this.game.phase === Phase.INITIALDRAFTING &&
        this.game.gameOptions.initialDraftVariant
      );
    },
    ...
    getHideButtonClass(hideType: string): string {
      ...
      if (hideType === "HAND") {
        return (
          prefix + (this.showHand ? "hand-toggle" : "hand-toggle-transparent")
        );
      } else if (hideType === "ACTIVE") {
        return (
  ...
  mounted() {
    ...
    if (
      this.game.isTerraformed &&
      TerraformedAlertDialog.shouldAlert &&
      getPreferences().show_alerts
    ) {
