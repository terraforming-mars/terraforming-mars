        ...
        <div id="player-home" :class="(player.game.turmoil ? 'with-turmoil': '')">
            ...
            <div v-if="thisPlayer.corporationCard">
                ...
                <div class="player_home_block nofloat">
                    <log-panel :id="player.id" :players="player.players" :generation="player.game.generation" :lastSoloGeneration="player.game.lastSoloGeneration" :color="thisPlayer.color"></log-panel>
                </div>
                ...
                <div class="player_home_block player_home_block--hand" v-if="player.cardsInHand.length + thisPlayer.preludeCardsInHand.length > 0" id="shortkey-hand">
                    <dynamic-title title="Cards In Hand" :color="thisPlayer.color" :withAdditional="true" :additional="(thisPlayer.cardsInHandNbr + thisPlayer.preludeCardsInHand.length).toString()" />
                    <sortable-cards :playerId="player.id" :cards="thisPlayer.preludeCardsInHand.concat(player.cardsInHand)" />
                </div>
                ...
                <div class="player_home_block player_home_block--cards">
                    ...
                    <div v-if="thisPlayer.corporationCard !== undefined" class="cardbox">
                        <Card :card="thisPlayer.corporationCard" :actionUsed="isCardActivated(thisPlayer.corporationCard, player.thisPlayer)"/>
                    </div>
                    <div v-show="isVisible('ACTIVE')" v-for="card in sortActiveCards(getCardsByType(thisPlayer.playedCards, [getActiveCardType()]))" :key="card.name" class="cardbox">
                        <Card :card="card" :actionUsed="isCardActivated(card, player.thisPlayer)"/>
                    </div>
                ...
                <template v-if="player.pickedCorporationCard.length === 1">
                  ...
                  <div>
                    ...
                    <template v-if="player.game.gameOptions.preludeExtension">
                      <div v-for="card in thisPlayer.preludeCardsInHand" :key="card.name" class="cardbox">
                        <Card :card="card"/>
                      </div>
                    </template>
                  </div>
