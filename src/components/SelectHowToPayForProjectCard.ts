...
export const SelectHowToPayForProjectCard = Vue.component('select-how-to-pay-for-project-card', {
  ...
  methods: {
    getCard: function() {
      const card = this.cards.find((c) => c.name === this.cardName); // this.playerView.cardsInHand.concat(this.playerView.selfReplicatingRobotsCards).find((c) => c.name === this.cardName);
      if (card === undefined) {
    ...
    setDefaultValues: function() {
      ...
      let megacreditBalance = Math.max(this.cost - this.playerView.me.megaCredits, 0);
      ...
      this.available.steel = Math.max(this.playerView.me.steel - this.card.reserveUnits.steel, 0);
      if (megacreditBalance > 0 && this.canUseSteel()) {
        this.steel = deductUnits(this.available.steel, this.playerView.me.steelValue, true);
      }

      this.available.titanium = Math.max(this.playerView.me.titanium - this.card.reserveUnits.titanium, 0);
      if (megacreditBalance > 0 && this.canUseTitanium()) {
        this.titanium = deductUnits(this.available.titanium, this.playerView.me.titaniumValue, true);
      }

      this.available.heat = Math.max(this.playerView.me.heat - this.card.reserveUnits.heat, 0);
      ...
      if (megacreditBalance < 0) {
        ...
        this.steel -= saveOverSpendingUnits(this.steel, this.playerView.me.steelValue);
    ...
    canUseHeat: function(): boolean {
      return this.playerinput.canUseHeat === true && this.playerView.me.heat > 0;
    },
    ...
    saveData: function() {
      ...
      const totalSpentAmt =
        ...
        htp.heat +
        htp.megaCredits +
        (htp.steel * this.playerView.me.steelValue) +
        (htp.titanium * this.playerView.me.titaniumValue);
