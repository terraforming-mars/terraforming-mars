...
import {
  AltSecondaryTag,
  CardRenderItem,
} from '../../cards/render/CardRenderItem';
...
export const CardRenderItemComponent = Vue.component(
    ...
    methods: {
      getComponentClasses: function (): string {
        ...
        if (type === CardRenderItemType.TEMPERATURE) {
          ...
          type === CardRenderItemType.TRADE_DISCOUNT ||
          type === CardRenderItemType.MULTIPLIER_WHITE
        ) {
          classes.push('card-resource');
        ...
        if (this.item.type === CardRenderItemType.MOON_COLONY_RATE) {
          classes.push(
            this.item.size === undefined
              ? 'card-colony-rate'
              : `card-colony-rate--${this.item.size}`
          );
        }
        ...
        if (this.item.type === CardRenderItemType.MOON_MINING_RATE) {
          classes.push(
            this.item.size === undefined
              ? 'card-mining-rate'
              : `card-mining-rate--${this.item.size}`
          );
        }
        ...
        if (this.item.type === CardRenderItemType.MOON_LOGISTICS_RATE) {
          classes.push(
            this.item.size === undefined
              ? 'card-logistics-rate'
              : `card-logistics-rate--${this.item.size}`
          );
        }
      ...
      itemHtmlContent: function (): string {
        ...
        if (
          this.item.secondaryTag !== undefined &&
          !previouslyRendered.includes(this.item.secondaryTag)
        ) {
        ...
        if (
          this.item.type === CardRenderItemType.NO_TAGS ||
          this.item.type === CardRenderItemType.MULTIPLIER_WHITE
        ) {
          result = 'X';
        ...
        if (this.item.type === CardRenderItemType.SELF_REPLICATING) {
          result =
            '<div class="card-resource card-card"><div class="cards-count">2</div><div class="card-icon card-icon-space">✴</div><div class="card-icon card-icon-building">☗</div></div>';
        }
        ...
        if (this.item.type === CardRenderItemType.PRELUDE) {
          result =
            '<div class="card-prelude-container"><span class="card-prelude-icon">prel</span></div>';
        }
        ...
        if (this.item.type === CardRenderItemType.VP) {
          result =
            '<div class="card-resource points-big card-vp-questionmark">?</div>';
        }
        // TODO(chosta): find a reasonable way to represent "?" (alphanumeric maybe)
        // This is assocaited with the card Playwrights.
        if (
          this.item.type === CardRenderItemType.MEGACREDITS &&
          this.item.amount === 1000
        ) {
          result = '?';
        }
        ...
        if (
          this.item.type === CardRenderItemType.TR &&
