...
export class PostgreSQL implements IDatabase {
  ...
  getGames(cb: (err: Error | undefined, allGames: Array<GameId>) => void) {
    ...
    const sql: string =
      "SELECT games.game_id FROM games, (SELECT max(save_id) save_id, game_id FROM games WHERE status='running' GROUP BY game_id) a WHERE games.game_id = a.game_id AND games.save_id = a.save_id ORDER BY created_time DESC";
    this.client.query(sql, (err, res) => {
      if (err) {
  ...
  getGameVersion(
    game_id: GameId,
    save_id: number,
    cb: DbLoadCallback<SerializedGame>
  ): void {
    this.client.query(
      'SELECT game game FROM games WHERE game_id = $1 and save_id = $2',
      [game_id, save_id],
      (err: Error | null, res: QueryResult<any>) => {
        if (err) {
  ...
  purgeUnfinishedGames(): void {
    ...
    this.client.query(
      "DELETE FROM games WHERE created_time < now() - interval '1 day' * $1",
      [days],
      function (err?: Error, res?: QueryResult<any>) {
        if (res) {
  ...
  deleteGameNbrSaves(game_id: GameId, rollbackCount: number): void {
    if (rollbackCount > 0) {
      this.client.query(
        'DELETE FROM games WHERE ctid IN (SELECT ctid FROM games WHERE game_id = $1 ORDER BY save_id DESC LIMIT $2)',
        [game_id, rollbackCount],
        (err) => {
          if (err) {
            return console.warn(err.message);
          }
