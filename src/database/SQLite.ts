...
export class SQLite implements IDatabase {
  ...
  getGames(cb: (err: Error | undefined, allGames: Array<GameId>) => void) {
    ...
    const sql: string =
      "SELECT distinct game_id game_id FROM games WHERE status = 'running'";
    this.db.all(sql, [], (err, rows) => {
      if (rows) {
        rows.forEach((row) => {
  ...
  loadCloneableGame(game_id: GameId, cb: DbLoadCallback<SerializedGame>) {
    ...
    this.db.get(
      ...
      [game_id],
      (err: Error | null, row: {game_id: GameId; game: any}) => {
        ...
        try {
          ...
          console.error(
            `unable to load game ${game_id} at save point 0`,
            exception
          );
  ...
  deleteGameNbrSaves(game_id: GameId, rollbackCount: number): void {
    if (rollbackCount > 0) {
      this.db.run(
        'DELETE FROM games WHERE rowid IN (SELECT rowid FROM games WHERE game_id = ? ORDER BY save_id DESC LIMIT ?)',
