...
export class DrawCards<T extends undefined | SelectCard<IProjectCard>>
  implements DeferredAction
{
  ...
  public execute(): T {
    ...
    const cards = game.dealer.drawProjectCardsByCondition(
      game,
      this.count,
      (card) => {
        if (
          this.options.resource !== undefined &&
          this.options.resource !== card.resourceType
        ) {
          return false;
        }
        if (
          this.options.cardType !== undefined &&
          this.options.cardType !== card.cardType
        ) {
          return false;
        }
        if (
          this.options.tag !== undefined &&
          !card.tags.includes(this.options.tag)
        ) {
          return false;
        }
  ...
  public static keepAll(
    player: Player,
    count: number = 1,
    options?: DrawCards.DrawOptions
  ): DrawCards<undefined> {
    return new DrawCards(player, count, options, (cards) =>
      DrawCards.keep(
        player,
        cards,
        options === undefined ? LogType.DREW : LogType.DREW_VERBOSE
      )
    );
  }
  ...
  ): DrawCards<SelectCard<IProjectCard>> {
    return new DrawCards(player, count, options, (cards) =>
      DrawCards.choose(player, cards, options)
    );
  }
  ...
  public static discard(
    player: Player,
    preserve: Array<IProjectCard>,
    discard: Array<IProjectCard>
  ) {
  ...
  public static choose(
    player: Player,
    cards: Array<IProjectCard>,
    options: DrawCards.ChooseOptions
  ): SelectCard<IProjectCard> {
    let max = options.keepMax || cards.length;
    if (options.paying) {
      max = Math.min(
        max,
        Math.floor(player.spendableMegacredits() / player.cardCost)
      );
    }
    const min = options.paying ? 0 : options.keepMax;
    const msg = options.paying
      ? max === 0
    ...
    const cb = (selected: Array<IProjectCard>) => {
      if (options.paying && selected.length > 0) {
        ...
        this.keep(
          player,
          selected,
          options.paying ? LogType.BOUGHT : LogType.DREW
        );
        this.discard(player, selected, cards);
      }
      return undefined;
