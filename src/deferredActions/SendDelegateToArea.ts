...
export class SendDelegateToArea implements DeferredAction {
  ...
  public execute() {
    ...
    if (this.options.replace) {
      parties = turmoil.parties.filter((party) => {
        ...
        for (const delegate of party.delegates) {
          ...
          return (
            party.delegates.filter(
              (delegate) => delegate === this.options.replace
            ).length > 1
          );
        }
        return false;
    ...
    const sendDelegate = new SelectPartyToSendDelegate(
      this.title,
      'Send delegate',
      availableParties,
      (partyName: PartyName) => {
        if (this.options.cost) {
          this.player.game.defer(
            new SelectHowToPayDeferred(this.player, this.options.cost, {
              title: 'Select how to pay for send delegate action',
            })
          );
        }
        ...
        if (numDelegateToSend > 1 && source === 'lobby') {
          ...
          for (let i = 0; i < numDelegateToSend; i++) {
            if (this.options.replace) {
              turmoil.replaceDelegateFromParty(
        ...
        this.player.game.log('${0} sent ${1} delegate(s) in ${2} area', (b) =>
          b
            .player(this.player)
            .number(numDelegateToSend)
            .party(turmoil.getPartyByName(partyName))
        );
        return undefined;
      }
    );
