...
export class StealResources implements DeferredAction {
  ...
  constructor(
    ...
    public title: string = 'Select player to steal up to ' +
      count +
      ' ' +
      resource +
      ' from'
  ...
  public execute() {
    ...
    let candidates: Array<Player> = this.player.game
      .getPlayers()
      .filter(
        (p) => p.id !== this.player.id && p.getResource(this.resource) > 0
      );
    if (this.resource === Resources.PLANTS) {
      candidates = candidates.filter((p) => !p.plantsAreProtected());
    }
    if (
      this.resource === Resources.STEEL ||
      this.resource === Resources.TITANIUM
    ) {
    ...
    const stealOptions = candidates.map((candidate) => {
      const qtyToSteal = Math.min(
        candidate.getResource(this.resource),
        this.count
      );
      return new SelectOption(
        ...
        'Steal',
        () => {
          candidate.deductResource(this.resource, qtyToSteal, {
            log: true,
            from: this.player,
          });
