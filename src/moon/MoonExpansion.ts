...
export class MoonExpansion {
  ...
  public static ifMoon<T>(
    game: Game,
    cb: (moonData: IMoonData) => T
  ): T | undefined {
    if (game.gameOptions.moonExpansion) {
      if (game.moonData === undefined) {
        console.log(
          `Assertion failure: game.moonData is undefined for ${game.id}`
        );
      } else {
  ...
  public static moonData(game: Game): IMoonData {
    ...
    throw new Error(
      'Assertion error: Using a Moon feature when the Moon expansion is undefined.'
    );
  }
  ...
  public static addMineTile(
    ...
    MoonExpansion.addTile(player, spaceId, {
      tileType: TileType.MOON_MINE,
      card: cardName,
    });
  }
  ...
  public static addTile(player: Player, spaceId: string, tile: ITile): void {
    ...
    MoonExpansion.ifMoon(game, (moonData) => {
      ...
      if (
        ...
        player.corporationCard.onTilePlaced(
          player,
          player,
          space,
          BoardType.MOON
  ...
  private static logTilePlacement(
    player: Player,
    space: ISpace,
    tileType: TileType
  ) {
    ...
    if (space.x !== -1 && space.y !== -1) {
      ...
      player.game.log(
        ...
        (b) =>
          b
            .player(player)
            .string(TileType.toString(tileType))
            .number(row)
            .number(position)
      );
    }
  ...
  public static raiseMiningRate(player: Player, count: number = 1) {
    MoonExpansion.ifMoon(player.game, (moonData) => {
      ...
      if (increment > 0) {
        if (player.game.phase === Phase.SOLAR) {
          player.game.log(
            'The World Government raised the mining rate ${1} step(s)',
            (b) => b.player(player).number(increment)
          );
          this.activateLunaFirst(undefined, player.game, increment);
        } else {
          player.game.log('${0} raised the mining rate ${1} step(s)', (b) =>
            b.player(player).number(increment)
          );
  ...
  public static raiseColonyRate(player: Player, count: number = 1) {
    MoonExpansion.ifMoon(player.game, (moonData) => {
      ...
      if (increment > 0) {
        if (player.game.phase === Phase.SOLAR) {
          player.game.log(
            'The World Government raised the colony rate ${1} step(s)',
            (b) => b.player(player).number(increment)
          );
          this.activateLunaFirst(undefined, player.game, count);
        } else {
          player.game.log(
            '${0} raised the moon colony rate ${1} step(s)',
            (b) => b.player(player).number(increment)
          );
  ...
  public static raiseLogisticRate(player: Player, count: number = 1) {
    MoonExpansion.ifMoon(player.game, (moonData) => {
      ...
      if (increment > 0) {
        if (player.game.phase === Phase.SOLAR) {
          player.game.log(
            'The World Government raised the logistic rate ${1} step(s)',
            (b) => b.player(player).number(increment)
          );
          this.activateLunaFirst(undefined, player.game, increment);
        } else {
          player.game.log('${0} raised the logistic rate ${1} step(s)', (b) =>
            b.player(player).number(increment)
          );
          player.increaseTerraformRatingSteps(count);
  ...
  private static activateLunaFirst(
    ...
    if (lunaFirstPlayer !== undefined) {
      ...
      if (lunaFirstPlayer.id === sourcePlayer?.id) {
        lunaFirstPlayer.addProduction(Resources.MEGACREDITS, count, {
          log: true,
        });
      }
    }
   ...
   */
  public static adjustedReserveCosts(
    player: Player,
    card: IProjectCard
  ): Units {
    if (
      player.cardIsInEffect(CardName.LTF_PRIVILEGES) &&
      card.tags.includes(Tags.MOON)
    ) {
      return Units.EMPTY;
    }
    ...
    const tilesBuilt: Array<TileType> =
      (card as unknown as IMoonCard).tilesBuilt || [];

    if (
      tilesBuilt.includes(TileType.MOON_COLONY) &&
      player.cardIsInEffect(CardName.SUBTERRANEAN_HABITATS)
    ) {
      titanium -= 1;
    }

    if (
      tilesBuilt.includes(TileType.MOON_MINE) &&
      player.cardIsInEffect(CardName.IMPROVED_MOON_CONCRETE)
    ) {
      titanium -= 1;
    }

    if (
      tilesBuilt.includes(TileType.MOON_ROAD) &&
      player.cardIsInEffect(CardName.LUNAR_DUST_PROCESSING_PLANT)
    ) {
      steel = 0;
    }
  ...
  public static calculateVictoryPoints(
    player: Player,
    vpb: VictoryPointsBreakdown
  ): void {
    MoonExpansion.ifMoon(player.game, (moonData) => {
      ...
      const mySpaces = moon.spaces.filter(
        (space) => space.player?.id === player.id
      );
          ...
          switch (type) {
            ...
            case TileType.LUNAR_MINE_URBANIZATION:
              const points = moon
                ...
                .filter((adj) =>
                  MoonExpansion.spaceHasType(adj, TileType.MOON_ROAD)
                ).length;
              if (
                type === TileType.MOON_MINE ||
                type === TileType.LUNAR_MINE_URBANIZATION
              ) {
                vpb.setVictoryPoints('moon mine', points);
              }
              if (
                type === TileType.MOON_COLONY ||
                type === TileType.LUNAR_MINE_URBANIZATION
              ) {
