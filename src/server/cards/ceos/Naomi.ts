  ...
  public action(player: IPlayer): PlayerInput | undefined {
    ...
    activeColonies.forEach((colony) => {
      game.defer(new SimpleDeferredAction(player, () => new OrOptions(
        new SelectOption('Move the ' + colony.name + ' tile track marker to its HIGHEST value', 'Select').andThen(() => {
          colony.trackPosition = MAX_COLONY_TRACK_POSITION;
          return undefined;
        }),
        new SelectOption('Move the ' + colony.name + ' tile track marker to its LOWEST value', 'Select').andThen(() => {
