...
export class Ryu extends CeoCard {
  ...
  public action(player: IPlayer): PlayerInput | undefined {
    ...
    ALL_RESOURCES.filter((r) => this.productionIsDecreasable(player, r)).forEach((resourceToDecrease) => {
      const selectOption = new SelectOption(newMessage('Decrease ${0} production', (b) => b.string(resourceToDecrease)), 'Select').andThen(() => {
        ...
        return new SelectAmount(
          ...
          const productionToIncrease =
            ALL_RESOURCES.filter((res) => res !== resourceToDecrease)
              .map((res) => new SelectOption(newMessage('Increase ${0} production', (b) => b.string(res)), 'Select').andThen(() => {
