  ...
  public action(player: IPlayer) {
    ...
    MoonExpansion.ifMoon(player.game, (moonData) => {
      if (this.canRaiseHabitatRate(player) && moonData.habitatRate < 8) {
        options.push(new SelectOption('Spend 2 microbes to raise the habitat rate 1 step.', 'Select').andThen(() => {
    ...
    options.push(new SelectOption('Add 1 microbe to this card', 'Select').andThen(() => {
