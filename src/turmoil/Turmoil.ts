...
} from './globalEvents/GlobalEventDealer';
...
export class Turmoil implements ISerializable<SerializedTurmoil> {
  ...
  public politicalAgendasData: PoliticalAgendasData =
    UNINITIALIZED_POLITICAL_AGENDAS_DATA;
  ...
  public static newInstance(
    game: Game,
    agendaStyle: AgendaStyle = AgendaStyle.STANDARD
  ): Turmoil {
    ...
    turmoil.politicalAgendasData = PoliticalAgendas.newInstance(
      agendaStyle,
      turmoil.parties
    );
  ...
  public initGlobalEvent(game: Game) {
    ...
    if (this.comingGlobalEvent !== undefined) {
      this.sendDelegateToParty(
        'NEUTRAL',
        this.comingGlobalEvent.revealedDelegate,
        game
      );
    }
    ...
    if (this.distantGlobalEvent !== undefined) {
      this.sendDelegateToParty(
        'NEUTRAL',
        this.distantGlobalEvent.revealedDelegate,
        game
      );
    }
  }
  ...
  public endGeneration(game: Game): void {
    ...
    if (this.currentGlobalEvent !== undefined) {
      ...
      game.log('Resolving global event ${0}', (b) =>
        b.globalEvent(currentGlobalEvent)
      );
    ...
    if (this.currentGlobalEvent) {
      this.globalEventDealer.discardedGlobalEvents.push(
    ...
    if (this.currentGlobalEvent) {
      this.sendDelegateToParty(
        'NEUTRAL',
        this.currentGlobalEvent.currentDelegate,
        game
      );
    }
  ...
  public setRulingParty(game: Game): void {
    if (this.rulingParty !== undefined) {
      ...
      game
        .getPlayers()
        .forEach((player) => (player.hasTurmoilScienceTagBonus = false));
      ...
      const index = this.rulingParty.delegates.indexOf(
        this.rulingParty.partyLeader!
      );
      // Remove the party leader from the delegates array
      this.rulingParty.delegates.splice(index, 1);
      // Fill the delegate reserve
      this.delegateReserve = this.delegateReserve.concat(
      ...
      if (this.chairman !== 'NEUTRAL') {
        ...
        const steps =
          player.corporationCard?.name === CardName.TEMPEST_CONSULTANCY ? 2 : 1;

        // Raise TR but after resolving the new policy
        game.defer(
          new DeferredAction(player, () => {
            player.increaseTerraformRatingSteps(steps);
            game.log('${0} is the new chairman and gains ${1} TR', (b) =>
              b.player(player).number(steps)
            );
            return undefined;
          })
        );
  ...
  public onAgendaSelected(game: Game): void {
    ...
    if (bonus === undefined) {
      throw new Error(
        `Bonus id ${bonusId} not found in party ${rulingParty.name}`
      );
    }
    ...
    if (policy === undefined) {
      throw new Error(
        `Policy id ${policyId} not found in party ${rulingParty.name}`
      );
    }
  ...
  public serialize(): SerializedTurmoil {
    const result: SerializedTurmoil = {
      ...
      politicalAgendasData: PoliticalAgendas.serialize(
  ...
  public static deserialize(d: SerializedTurmoil): Turmoil {
    ...
    turmoil.politicalAgendasData = PoliticalAgendas.deserialize(
      d.politicalAgendasData
    );
    ...
    turmoil.playersInfluenceBonus = new Map<string, number>(
      d.playersInfluenceBonus
    );
