...
export class TurmoilHandler {
  ...
  public static addPlayerAction(player: Player, options: PlayerInput[]): void {
    ...
    if (PartyHooks.shouldApplyPolicy(player.game, PartyName.SCIENTISTS)) {
      ...
      if (scientistsPolicy.canAct(player)) {
        options.push(
          new SelectOption(scientistsPolicy.description, 'Pay', () =>
    ...
    if (PartyHooks.shouldApplyPolicy(player.game, PartyName.KELVINISTS)) {
      ...
      if (kelvinistsPolicy.canAct(player)) {
        options.push(
          new SelectOption(kelvinistsPolicy.description, 'Pay', () =>
    ...
    // Turmoil Kelvinists action
    if (
      PartyHooks.shouldApplyPolicy(player.game, PartyName.KELVINISTS, 'kp03')
    ) {
    ...
    if (
      ...
      if (greensPolicy.canAct(player)) {
        options.push(
          new SelectOption(greensPolicy.description, 'Pay', () =>
    ...
    if (
      PartyHooks.shouldApplyPolicy(
        player.game,
        PartyName.MARS,
        TurmoilPolicy.MARS_FIRST_POLICY_4
      )
    ) {
      ...
      if (marsFirstPolicy.canAct(player)) {
        options.push(
          new SelectOption(marsFirstPolicy.description, 'Pay', () =>
    ...
    if (
      ...
      if (unityPolicy.canAct(player)) {
        options.push(
          new SelectOption(unityPolicy.description, 'Pay', () =>
    ...
    if (
      ...
      if (redsPolicy.canAct(player)) {
        options.push(
          new SelectOption(redsPolicy.description, 'Pay', () =>
  ...
  public static applyOnCardPlayedEffect(
    player: Player,
    ...
    if (
      PartyHooks.shouldApplyPolicy(
        player.game,
        PartyName.MARS,
        TurmoilPolicy.MARS_FIRST_POLICY_2
      )
    ) {
  ...
  public static resolveTilePlacementBonuses(
    player: Player,
    spaceType: SpaceType
  ): void {
    ...
    if (
      PartyHooks.shouldApplyPolicy(
        ...
        TurmoilPolicy.KELVINISTS_POLICY_4
      )
    ) {
  ...
  public static onGlobalParameterIncrease(
    player: Player,
    ...
    if (parameter === GlobalParameter.TEMPERATURE) {
      ...
      if (
        PartyHooks.shouldApplyPolicy(
          player.game,
          PartyName.KELVINISTS,
          TurmoilPolicy.KELVINISTS_POLICY_2
        )
      ) {
    ...
    if (
      PartyHooks.shouldApplyPolicy(
        ...
        TurmoilPolicy.SCIENTISTS_POLICY_3
      )
    ) {
