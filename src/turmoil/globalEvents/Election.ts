...
export class Election implements IGlobalEvent {
  ...
  public resolve(game: Game, turmoil: Turmoil) {
    ...
    if (game.isSoloMode()) {
      ...
      const players = [...game.getPlayers()].sort(
        (p1, p2) =>
          this.getScore(p2, turmoil, game) - this.getScore(p1, turmoil, game)
      );

      // We have one rank 1 player
      if (
        this.getScore(players[0], turmoil, game) >
        this.getScore(players[1], turmoil, game)
      ) {
        ...
        if (players.length === 1) {
          ...
          if (
            ...
            while (
              players.length > 0 &&
              this.getScore(players[0], turmoil, game) === score
            ) {
              players[0].increaseTerraformRatingSteps(1);
              LogHelper.logTRIncrease(players[0], 1);
              players.shift();
  ...
  public getScore(player: Player, turmoil: Turmoil, game: Game) {
    const score =
      player.getTagCount(Tags.BUILDING, false, false) +
