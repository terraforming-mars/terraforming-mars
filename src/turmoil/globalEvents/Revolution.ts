...
export class Revolution implements IGlobalEvent {
  ...
  public resolve(game: Game, turmoil: Turmoil) {
    if (game.isSoloMode()) {
      ...
      if (
        this.getScore(players[0], turmoil) > this.getScore(players[1], turmoil)
      ) {
        ...
        if (players.length === 1 && this.getScore(players[0], turmoil) > 0) {
          ...
          if (
            this.getScore(players[0], turmoil) >
            this.getScore(players[1], turmoil)
          ) {
            players[0].decreaseTerraformRating();
            ...
            while (
              players.length > 0 &&
              this.getScore(players[0], turmoil) === score
            ) {
  ...
  public getScore(player: Player, turmoil: Turmoil) {
    return (
      player.getTagCount(Tags.EARTH, false, false) +
