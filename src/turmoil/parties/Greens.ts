...
export class Greens extends Party implements IParty {
    ...
    public rulingBonus(game: Game): void {
        game.getPlayers().forEach(player => {
            let tagCount = player.getTagCount(Tags.PLANT, false, false) + player.getTagCount(Tags.MICROBES, false, false) + player.getTagCount(Tags.ANIMAL, false, false) + player.getTagCount(Tags.WILDCARD);
