...
import {
  ...
  MIN_TEMPERATURE,
  MIN_VENUS_SCALE,
  POLITICAL_AGENDAS_MAX_ACTION_USES,
} from '../../constants';
...
class RedsBonus02 implements Bonus {
  ...
  grant(game: Game) {
    ...
    if (game.isSoloMode() && players[0].getTerraformRating() > 20) {
      ...
      players.sort(
        (p1, p2) => p2.getTerraformRating() - p1.getTerraformRating()
      );
...
class RedsPolicy02 implements Policy {
  ...
  onTilePlaced(player: Player) {
    ...
    if (amountToPay > 0) {
      player.game.defer(
        new SelectHowToPayDeferred(player, amountToPay, {
          title: 'Select how to pay for tile placement',
        })
      );
    }
  }
    ...
    game.defer(
      new SelectHowToPayDeferred(player, 4, {
        ...
        afterPay: () => {
          ...
          const temperature = game.getTemperature();
          const canDecreaseTemperature =
            temperature > MIN_TEMPERATURE && temperature !== MAX_TEMPERATURE;

          if (canDecreaseTemperature) {
            orOptions.options.push(
              new SelectOption('Decrease temperature', 'Confirm', () => {
                game.increaseTemperature(player, -1);
                game.log('${0} decreased temperature 1 step', (b) =>
                  b.player(player)
                );
                return undefined;
              })
          ...
          // Remove ocean option
          const oceansPlaced = game.board.getOceansOnBoard();
          const canRemoveOcean =
            oceansPlaced > 0 && oceansPlaced !== MAX_OCEAN_TILES;

          if (canRemoveOcean) {
            orOptions.options.push(
              new SelectOption('Remove an ocean tile', 'Confirm', () => {
                game.defer(
                  new RemoveOceanTile(
                    player,
                    'Turmoil Reds action - Remove an Ocean tile from the board'
                  )
                );
                return undefined;
              })
          ...
          const oxygenLevel = game.getOxygenLevel();
          const canDecreaseOxygen =
            oxygenLevel > MIN_OXYGEN_LEVEL && oxygenLevel !== MAX_OXYGEN_LEVEL;

          if (canDecreaseOxygen) {
            orOptions.options.push(
              new SelectOption('Decrease oxygen level', 'Confirm', () => {
                game.increaseOxygenLevel(player, -1);
                game.log('${0} decreased oxygen level 1 step', (b) =>
                  b.player(player)
                );
                return undefined;
              })
          ...
          const venusScaleLevel = game.getVenusScaleLevel();
          const canDecreaseVenus =
            game.gameOptions.venusNextExtension === true &&
            venusScaleLevel > MIN_VENUS_SCALE &&
            venusScaleLevel !== MAX_VENUS_SCALE;

          if (canDecreaseVenus) {
            orOptions.options.push(
              new SelectOption('Decrease Venus scale', 'Confirm', () => {
                game.increaseVenusScaleLevel(player, -1);
                game.log('${0} decreased Venus scale level 1 step', (b) =>
                  b.player(player)
                );
                return undefined;
              })
