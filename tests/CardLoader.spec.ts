...
describe('CardLoader', function () {
  it('correctly removes projectCardsToRemove', function () {
    ...
    const names = new CardLoader(gameOptions)
      .getProjectCards()
      .map((c) => c.name);
  ...
  it('correctly separates 71 corporate era cards', function () {
    ...
    expect(new CardLoader(gameOptions).getProjectCards().length).to.eq(208);

    // exclude corporate era
    gameOptions.corporateEra = false;
    expect(new CardLoader(gameOptions).getProjectCards().length).to.eq(137);
  });
  ...
  it('excludes expansion-specific preludes if those expansions are not selected ', function () {
    ...
    COMMUNITY_CARD_MANIFEST.preludeCards.factories.forEach((cf) =>
      turmoilPreludes.push(cf.cardName)
    );
