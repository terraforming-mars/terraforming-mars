...
import {
  ...
  ORIGINAL_MILESTONES,
  VENUS_MILESTONES,
} from '../src/milestones/Milestones';
...
describe('MilestoneAwardSelector', () => {
  ...
  it('Tharsis milestones and awards have high synergy', () => {
    ...
    expect(
      MilestoneAwardSelector.maximumSynergy(
  ...
  it('Venus milestones and awards have high synergy', () => {
    ...
    expect(
      MilestoneAwardSelector.maximumSynergy(
        toNames([...VENUS_MILESTONES, ...VENUS_AWARDS])
      )
    ).eq(5);
  });
  ...
  it('Tharsis milestones and awards break limited synergy rules', () => {
    ...
    expect(
      MilestoneAwardSelector.verifySynergyRules(
        toNames([...ORIGINAL_MILESTONES, ...ORIGINAL_AWARDS]),
        MilestoneAwardSelector.LIMITED_SYNERGY
      )
    ).eq(false);
  });
  ...
  it('Do not select expansion milestones or awards when they are not selected', () => {
    ...
    const avoidedAwards = [...VENUS_AWARDS, ...ARES_AWARDS, ...MOON_AWARDS].map(
      (a) => a.name
    );
    ...
    for (let idx = 0; idx < 10000; idx++) {
      ...
      expect(
        intersection(
          mas.awards.map((award) => award.name),
          avoidedAwards
        )
      ).is.empty;
      expect(
        intersection(
          mas.milestones.map((milestone) => milestone.name),
          avoidedMilestones
        )
      ).is.empty;
    }
