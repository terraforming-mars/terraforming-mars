...
describe('AresHandler', function () {
  ...
  it('setupHazards', function () {
    ...
    expect(spacesWithTiles).to.deep.eq([
      {
        tile: {tileType: TileType.DUST_STORM_MILD, protectedHazard: false},
        x: 8,
        y: 0,
      },
      {
  ...
  it('Pay adjacent hazard costs - mild', function () {
    ...
    try {
      ...
      expect(err.toString()).includes(
        'Placing here costs 1 units of production'
      );
    }
    ...
    const input = game.deferredActions
      .peek()!
      .execute() as SelectProductionToLose;
  ...
  it('pay adjacent hazard costs - severe', function () {
    ...
    try {
      ...
      expect(err.toString()).includes(
        'Placing here costs 2 units of production'
      );
    }
  ...
  it("Placing on top of an ocean doesn't regrant bonuses", function () {
    ...
    game.addTile(player, SpaceType.OCEAN, space, {
