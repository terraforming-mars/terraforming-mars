...
describe('MarsBotBoard', () => {
  describe('tag mapping', () => {
    ...
    it('has 7 tracks', () => {
      expect(marsBotBoard.tracks.length).to.eq(7);
    });

    it('maps Building tag to track 0', () => {
      expect(marsBotBoard.tagToTrack[Tag.BUILDING]).to.eq(0);
    });

    it('maps Space tag to track 1', () => {
      expect(marsBotBoard.tagToTrack[Tag.SPACE]).to.eq(1);
    });

    it('maps Event tag to track 2', () => {
      expect(marsBotBoard.tagToTrack[Tag.EVENT]).to.eq(2);
    });

    it('maps Science tag to track 3', () => {
      expect(marsBotBoard.tagToTrack[Tag.SCIENCE]).to.eq(3);
    });

    it('maps Power/Jovian to track 4', () => {
      expect(marsBotBoard.tagToTrack[Tag.POWER]).to.eq(4);
      expect(marsBotBoard.tagToTrack[Tag.JOVIAN]).to.eq(4);
    });

    it('maps Earth/City to track 5', () => {
      expect(marsBotBoard.tagToTrack[Tag.EARTH]).to.eq(5);
      expect(marsBotBoard.tagToTrack[Tag.CITY]).to.eq(5);
    });

    it('maps Plant/Animal/Microbe to track 6', () => {
      expect(marsBotBoard.tagToTrack[Tag.PLANT]).to.eq(6);
      expect(marsBotBoard.tagToTrack[Tag.ANIMAL]).to.eq(6);
      expect(marsBotBoard.tagToTrack[Tag.MICROBE]).to.eq(6);
    });

    it('leaves unmapped tags without a track', () => {
      expect(marsBotBoard.tagToTrack[Tag.VENUS]).to.be.undefined;
      expect(marsBotBoard.tagToTrack[Tag.WILD]).to.be.undefined;
    });
  });
  ...
  describe('MarsBotTrack', () => {
    ...
    it('peek after regression still shows the layout action', () => {
      ...
      // peek does NOT check regression markers, it shows the raw layout.
      expect(track.peek()).to.eq('ocean');
    });
  });

  describe('with the Venus track', () => {
    let marsBotBoard: MarsBotBoard;

    beforeEach(() => {
      marsBotBoard = new MarsBotBoard([...THARSIS_MARSBOT_BOARD, VENUS_MARSBOT_TRACK]);
    });

    it('has 8 tracks', () => {
      expect(marsBotBoard.tracks.length).to.eq(8);
    });

    it('maps Venus tag to track 7', () => {
      expect(marsBotBoard.tagToTrack[Tag.VENUS]).to.eq(7);
    });

    it('Venus track maxes out at position 12', () => {
      const track = marsBotBoard.tracks[7];
      for (let i = 0; i < 12; i++) {
    ...
    it('main tracks still max out at position 18', () => {
      const track = marsBotBoard.tracks[0];
      for (let i = 0; i < 18; i++) {
        track.advance();
      }
      expect(track.position).to.eq(18);
