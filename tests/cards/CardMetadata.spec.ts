    ...
    ALL_CARD_MANIFESTS.forEach((manifest) => {
      manifest.projectCards.factories.forEach((c) => {
        ...
        if (
          ...
          expect(
            card.metadata.victoryPoints,
            card.name + ' is missing VP metadata'
          ).is.not.undefined;
          const vp = card.getVictoryPoints(player);
          if (vp !== 0) {
            if (
              card.metadata.victoryPoints instanceof
                CardRenderDynamicVictoryPoints &&
              card.metadata.victoryPoints.anyPlayer === true
            ) {
              expect(
                card.metadata.victoryPoints.points,
                card.name + ' has invalid VP metadata'
              ).to.eq(vp);
            } else {
              expect(
                card.metadata.victoryPoints,
