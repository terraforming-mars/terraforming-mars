...
describe('AerospaceMission', function () {
  ...
  it('Should play', function () {
    ...
    selectColony.cb(
      (<any>ColonyName)[selectColony.coloniesModel[0].name.toUpperCase()]
    );
    expect(player.getProduction(Resources.ENERGY)).to.eq(1);

    // Build the second free on Ceres
    const selectColony2 = game.deferredActions
      .peek()!
      .execute() as SelectColony;
    game.deferredActions.pop();
    selectColony2.cb(
      (<any>ColonyName)[selectColony2.coloniesModel[0].name.toUpperCase()]
    );
    expect(player.getProduction(Resources.STEEL)).to.eq(1);

    // Check that we built two colonies
    const builtColonies = game.colonies.filter(
      (colony) => colony.isActive && colony.colonies.length > 0
    );
    expect(builtColonies).has.lengthOf(2);
    expect(builtColonies[0].colonies.find((c) => c === player.id)).is.not
      .undefined;
    expect(builtColonies[1].colonies.find((c) => c === player.id)).is.not
      .undefined;
  });
});
