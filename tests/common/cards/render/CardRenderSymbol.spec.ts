...
describe('CardRenderSymbol', () => {
  it('asterix: success', () => {
    ...
    expect(symbol.size).to.equal(Size.MEDIUM);
    expect(symbol.type).to.equal(CardRenderSymbolType.ASTERIX);
  });
  it('or: success', () => {
    const symbol = CardRenderSymbol.or(Size.SMALL);
    expect(symbol.size).to.equal(Size.SMALL);
  ...
  it('minus: success', () => {
    const symbol = CardRenderSymbol.minus(Size.SMALL);
    expect(symbol.size).to.equal(Size.SMALL);
    expect(symbol.isIcon).to.be.true;
  ...
  it('slash: success', () => {
    const symbol = CardRenderSymbol.slash(Size.SMALL);
    expect(symbol.size).to.equal(Size.SMALL);
  ...
  it('nbsp: success', () => {
    const symbol = CardRenderSymbol.nbsp(Size.SMALL);
    expect(symbol.size).to.equal(Size.SMALL);
    expect(symbol.isIcon).to.be.true;
