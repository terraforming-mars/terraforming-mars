...
describe('GameLoader', function () {
  ...
  before(function () {
    ...
    const database = {
      getGame: function (
        gameId: string,
        theCb: (err: unknown, serializedGame?: SerializedGame) => void
      ) {
        if (gameId === 'foobar') {
  ...
  it('gets game when requested before database loaded', function (done) {
    ...
    Database.getInstance().getGames = function (
      cb: (err: any, games: Array<string>) => void
    ) {
      setTimeout(function () {
        cb(undefined, ['foobar']);
      });
  ...
  it('gets no game when game goes missing from database', function () {
    ...
    GameLoader.getInstance().getByGameId('never', false, (game1) => {
      expect(game1).is.undefined;
      Database.getInstance().getGame = function (
        _gameId: string,
        cb: (err: any, serializedGame?: SerializedGame) => void
      ) {
  ...
  it('gets player when it exists in database', function () {
    ...
    GameLoader.getInstance().getByPlayerId(
