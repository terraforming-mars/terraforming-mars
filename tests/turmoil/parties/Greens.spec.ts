...
import {
  ...
  GREENS_BONUS_2,
  GREENS_POLICY_4,
} from '../../../src/turmoil/parties/Greens';
...
describe('Greens', function () {
  ...
  it('Ruling policy 2: When you place a tile, gain 1 plant', function () {
    TestingUtils.setRulingPartyAndRulingPolicy(
      ...
      turmoil,
      greens,
      greens.policies[1].id
    );

    const emptySpace: ISpace = game.board.spaces.find(
      (space) => space.spaceType === SpaceType.LAND && space.bonus.length === 0
    ) as ISpace;
    game.addTile(player, emptySpace.spaceType, emptySpace, {
